...
class ByteArray {
  ...
  public:
    ...
    ByteArray& operator=(ByteArray const& right) {
        resize(right.size());
    ...
    void writeAt(size_t position, void const* rawDataPtr, size_t sizeInByte) {
        assert(m_length >= position + sizeInByte);
        std::copy_n(reinterpret_cast<char const*>(rawDataPtr), sizeInByte, getData() + position);
    }

    void append(void const* rawDataPtr, size_t sizeInByte) {
        auto i = size();
        resize(size() + sizeInByte);
        std::copy_n(reinterpret_cast<char const*>(rawDataPtr), sizeInByte, getData() + i);
    }
